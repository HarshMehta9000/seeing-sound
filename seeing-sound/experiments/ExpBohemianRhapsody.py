...
# Load MIDI file
mid = MidiFile(r"C:\Users\Dell\Downloads\Parker. Charlie - Donna Lee___WWW.MIDISFREE.COM.mid")
...
print("df_final_with_all_notes:")
print(df_final_with_all_notes.head())
...
fig_hd = px.density_heatmap(df_final, x='time_elapsed_minutes', y='note_name_octave',
                            nbinsx=40, nbinsy=12, title="Queen - Bohemian Rhapsody.mid (inDepth Visualization)",
                            labels={'time_elapsed_minutes': 'Duration (Minutes)', 'note_name_octave': 'Note'})
fig_hd.update_layout(height=600, width=800, plot_bgcolor='black', paper_bgcolor='black',
                     font=dict(color='white'))
fig_hd.show()

# Second Visualization (3D Scatter Plot with Time Duration in Minutes)
fig_3d_scatter = px.scatter_3d(df_final_with_all_notes, x='time_elapsed_minutes', y='note_name_octave', z='frequency_y',
                               color='velocity', size='frequency_y', hover_name='message_type',
                               title="Queen - Bohemian Rhapsody.mid (3D Midi File Analysis)",
                               labels={'time_elapsed_minutes': 'Duration (Minutes)', 'note_name_octave': 'Note', 'frequency_y': 'Note Count'})
